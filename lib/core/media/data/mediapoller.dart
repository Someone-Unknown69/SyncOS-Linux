...
class MediaPoller implements ILocalMediaInfo{
  ...
  final StreamController<MediaInfo> _metadataController = StreamController<MediaInfo>.broadcast();
  ...
  void _monitorPlayer(String name) {
    ...
    final sub = DBusSignalStream(_client!, 
                    ...
                    .listen((signal) => _updateMetadata(name, signal: signal));
      
    _subscriptions.add(sub);
    
    // Send initial state
    _updateMetadata(name);
  }

  Future<void> _updateMetadata(String name, {DBusSignal? signal}) async {
  ...
  Future<void> _control(Map<String, dynamic> args) async {
    ...
    try {
      ...
      else if (args['method'] == 'seek') {_seek(args["position"]); return;}
